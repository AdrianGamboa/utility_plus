...
class TransferPage extends StatefulWidget {
  const TransferPage({Key? key, required this.accountList, }) : super(key: key);
...
class _TransferPageState extends State<TransferPage> {
            ...
            onPressed: () {
             showDialog(
                ...
                builder: (BuildContext ctx) {
                  return DotransferPage(accountList: widget.accountList,);
                }).then((value){ 
              if(value!=false){
                setState(() {});
              }
    ...
    return TextButton(
          ...
          showDialog(
              ...
              builder: (BuildContext ctx) {
                return DotransferPage(
                    transferInfo: convertToAccount(transferList[index]), accountList: widget.accountList,);
              }).then((value){ 
              if(value!=false){
                setState(() {});
              }
        ...
        child: Column(
          children: [
              ...
              RichText(
                  text: TextSpan(
                    ...
                    TextSpan(
                     ...
                     TextSpan(
                        text: getAccountName(transferList[index]['cuentaDebita'].toString()),
                        style: const TextStyle(height: 1.5)),
                      TextSpan(
                        text: '   ->   '+getAccountName(transferList[index]['cuentaAcredita'].toString()),
                        style: const TextStyle(height: 1.5)),
                      TextSpan(
            ...
            const Divider(color: colorPrimary),
          ],
        ));
  }
  ...
  void popUpClick(String value, m.ObjectId transfer_) {
    if (value == 'Eliminar') {
      delete(transfer_).then((value) => setState(() {}));
    }
  }
